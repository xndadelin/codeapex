...
import { useChallengeAdmin } from "@/hooks/use-challenges";
...
export default function NewProblemPage() {
   const { id } = useParams()
   const { data: challenge, isLoading } = useChallengeAdmin(id as UUID)
   ...
   const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<FormData>({
        ...
        defaultValues: {
            ...
            category: challenge?.category || topics[0],
            tags: challenge?.tags.join(", ") || "",
            description: challenge?.description || "",
            constraints: challenge?.constraints || "",
            starterCode: challenge?.starter_code || "",
            isPublic: challenge?.is_public || false,
            timeLimit: challenge?.time_limit || 2000,
            memoryLimit: challenge?.memory_limit || 256,
            testCasesJson: challenge ? JSON.stringify(challenge?.test_cases?.map(tc => ({
                input: tc.input,
                output: tc.output,
                is_sample: tc.is_sample,
                id:  tc.id
            }))) : defaultTestCasesJson
        }
   })
                ...
                <section className="relative py-12 lg:py-16">
                    <div className="relative mx-auto max-w-4xl px-6">
                        ...
                        <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">
                            Edit <span className="text-primary">challenge</span>
                        </h1>
                        <p className="text-muted-foreground text-sm mb-10">
                            Edit coding challenge details.
                        </p>
